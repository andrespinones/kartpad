...
| Is an IPA included in the GitHub release? | **Yes.** `v0.4.2` includes separate audited unsigned ARM64 IPAs for iPhone/iPad and experimental Apple TV hardware bring-up. Both require local re-signing and user-supplied supported game data. |
...
| Are Android and Apple TV supported? | Apple TV has an experimental native hardware-bring-up IPA in `v0.4.2`, but it is not accepted as supported until physical testers complete the matrix. Native Android implementation is now an active, evidence-gated project; no Android APK or runtime claim exists yet. |
...
| Distribution | `v0.4.2` provides source plus separate unsigned iPhone/iPad and experimental tvOS community IPAs containing translated game logic. They contain no disc image, extracted game assets, Retro Rewind pack, saves, signing identity, or provisioning profile |
...
Download `KartPad-v0.4.2-ios-unsigned.ipa` and `SHA256SUMS` from the
[latest release](https://github.com/chrissotraidis/kartpad/releases/tag/v0.4.2).
...
The same release includes `KartPad-v0.4.2-tvos-unsigned.ipa` for a
...
Yes. `v0.4.2` provides separate unsigned iPhone/iPad and experimental
...
Yes. The 0.4.2 release opens with an Original / Retro Rewind chooser and
...
TV tester IPA is included in `v0.4.2`; it has passed build and package
