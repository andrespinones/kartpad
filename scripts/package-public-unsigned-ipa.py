...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="?",
        help="Output IPA path (defaults to artifacts/KartPad-v0.3.0-preview.2-unsigned.ipa)",
    )
    ...
    output = (
        args.output.resolve()
        if args.output
        else repo / "artifacts/KartPad-v0.3.0-preview.2-unsigned.ipa"
    )
    if subprocess.run(
    ...
    additional_entries = {
        ...
        "RELEASE_NOTES.md": repo / "docs/releases/v0.3.0-preview.2.md",
