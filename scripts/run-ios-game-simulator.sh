...
signed_root="$(mktemp -d /tmp/kartpad-simulator-install.XXXXXX)"
cleanup() {
  if [[ "${signed_root}" == /tmp/kartpad-simulator-install.* ]]; then
    rm -rf -- "${signed_root}"
  fi
}
trap cleanup EXIT
ditto "${app}" "${signed_root}/KartPad.app"
codesign --force --sign - --timestamp=none "${signed_root}/KartPad.app"
...
xcrun simctl install "${udid}" "${signed_root}/KartPad.app"
xcrun simctl launch --terminate-running-process "${udid}" dev.kartpad.app
