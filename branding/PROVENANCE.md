...
`KartPadIcon.svg` is the editable production master authored for this repository from that concept. `KartPadIcon-Dark.svg` and `KartPadIcon-Tinted.svg` are platform appearance variants. The deterministic export script renders these masters without a corner mask and checks for an opaque 1024×1024 output.
