...
**G6 — PPC/AArch64 semantics are exact** is the lowest unmet goal. G0–G5 pass. The architecture differential, stateful translated invalid/suppression fixture, and complete 10,836-function Mario Kart Wii translation surface are green; fused/conversion/paired exception details and translated callback execution remain open.
...
| G6 PPC/AArch64 semantics | In progress | 250,197-check arm64/x86 differential hashes match; Dolphin estimates, sanitizers, patched translator 573/573, translated invalid/conversion write suppression, and all 10,836 real-DOL emitted units pass; fused/estimate/paired flags and callback execution remain open |
| G7–G18 | Gated | Provisional direct-Metal translated command frame exists, but it is not Dawn/Aurora, GX, or a game frame |

## Known-good state

- Repository checkpoint: `73e6e4e` (`origin/main`), full real-DOL PPC translation surface baseline.
...
- Translator baseline: immutable upstream remains 570/570; KartPad's reproducible FPSCR lowering patch passes 573/573 on native arm64 with .NET SDK 8.0.130.
...
- Semantic subset: arm64/x86_64 complete 250,197 checks with state hash `0x817dafe156e3268c`; the translated fixture proves VXISI/VXCVI, FEX, FPRF, FI/FR/XX, and enabled-VE destination suppression. Evidence is `docs/artifacts/2026-08-28/g6-ppc-semantics.md` and `docs/SEMANTICS.md`.
...
- Basic scalar add/subtract/multiply/divide/sqrt and float-to-word conversion now classify Broadway causes and suppress enabled-exception writes. Fused, estimate, paired-lane exception state and scheduler/callback persistence still need the same proof. G6 remains open.
