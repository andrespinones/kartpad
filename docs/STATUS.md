...
| G6 PPC/AArch64 semantics | In progress | 250,202-check arm64/x86 differential hashes match; Dolphin estimates, sanitizers, patched translator 577/577, translated invalid/conversion/fused write suppression, and all 10,836 real-DOL emitted units pass; estimate/paired flags and callback execution remain open |
...
- Translator baseline: immutable upstream remains 570/570; KartPad's reproducible FPSCR lowering patch passes 577/577 on native arm64 with .NET SDK 8.0.130.
...
- Semantic subset: arm64/x86_64 complete 250,202 checks with state hash `0x8947f7ff3d2e35f4`; the translated fixture proves VXISI/VXCVI/VXIMZ, FEX, FPRF, FI/FR/XX, and enabled-VE destination suppression. Evidence is `docs/artifacts/2026-08-28/g6-ppc-semantics.md` and `docs/SEMANTICS.md`.
...
- Basic scalar and fused arithmetic plus float-to-word conversion now classify Broadway causes and suppress enabled-exception writes. Estimate and paired-lane exception state plus scheduler/callback persistence still need the same proof. G6 remains open.
