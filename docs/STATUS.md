...
| G6 PPC/AArch64 semantics | In progress | 250,220-check arm64/x86 differential hashes match; Dolphin estimates, sanitizers, patched translator 579/579, translated scalar suppression/paired estimates/exact compares, and all 10,836 real-DOL emitted units pass; paired arithmetic flags and callback execution remain open |
...
- Semantic subset: arm64/x86_64 complete 250,220 checks with state hash `0x5a58605df18e5d1e`; the translated fixture additionally proves ordered/unordered scalar and paired compare CR/FPCC/VXVC behavior. Evidence is `docs/artifacts/2026-08-28/g6-ppc-semantics.md` and `docs/SEMANTICS.md`.
...
- Basic scalar, fused, conversion, estimate, and compare operations now model the covered Broadway state; paired estimates aggregate lane exceptions and always write like hardware. Paired arithmetic/fused state plus scheduler/callback persistence still need proof. G6 remains open.
