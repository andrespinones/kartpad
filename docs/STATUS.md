...
| G6 PPC/AArch64 semantics | In progress | 250,188-check arm64/x86 differential hashes match; Dolphin estimates, sanitizers, patched translator 571/571, translated invalid-write suppression, and all 10,836 real-DOL emitted units pass; fused/conversion/paired flags and callback execution remain open |
...
- Translator baseline: immutable upstream remains 570/570; KartPad's reproducible FPSCR lowering patch passes 571/571 on native arm64 with .NET SDK 8.0.130.
...
- Semantic subset: arm64/x86_64 complete 250,188 checks with state hash `0x09ff7940379dd04a`; the translated fixture proves VXISI, FEX, FPRF, and enabled-VE destination suppression. Evidence is `docs/artifacts/2026-08-28/g6-ppc-semantics.md` and `docs/SEMANTICS.md`.
...
- Basic scalar add/subtract/multiply/divide/sqrt helpers now classify Broadway invalid subcauses and suppress enabled-exception writes. Fused, conversion, estimate, paired-lane exception state and scheduler/callback persistence still need the same proof. G6 remains open.
