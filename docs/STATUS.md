...
- Direct mobile WBFS import has crossed its native-library feasibility gate. A
  clean pinned Dolphin DiscIO build for arm64 iOS Simulator opened the supplied
  `RMCP01` revision-0 WBFS, enumerated 2,095 filesystem entries, exported Wii
  system data, and reproduced the accepted `main.dol` SHA-256. The portability
  changes and curl SDK workaround are serialized in two verified patches, the
  pinned reference checkout is clean, and the build has a fresh-directory
  reproduction script. Full `files/` extraction, owned UIKit progress/error
  handling, atomic activation, physical-iOS compilation, and in-app runtime
  play remain the open WBFS-import boundary. Evidence:
