...
- Full mobile runtime link: a clean upstream-patched build compiles all 29,065 base translated functions and links Aurora GX/Metal, SDL UIKit/CoreAudio, registration/dispatch, and the runtime/HLE into a 78,548,760-byte arm64 `IOSSIMULATOR` Mach-O. The exact binary SHA-256 is `1d970f1ae75b5b0c8f3287df89d02d9b1b38524960808aa867868d30c855315c`; this is a standalone compile/link proof, not a runnable UIKit game. Evidence: `docs/artifacts/2026-08-30/g14-full-runtime-link.md`.
...
The local `ref/sunpad` checkout is the direct implementation reference for the mobile touch interface and persistent three-dot menu. A byte-identical, pinned nine-file snapshot now runs directly in the native arm64 shell on both iPhone 17 Pro and iPad Pro 13-inch Simulator classes. Its exact-snapshot verifier, Mario Kart Classic-input adapter, original icon catalog, privacy manifest, linked checked-memory/scheduler/host/translated startup self-check, scene lifecycle, rotation policy, compact-menu scrolling, settings/layout-editor reachability, and package audit pass. Both classes were exercised sequentially with the first fully terminated and shut down before the second booted; no device remains booted. The same unsigned shell builds and audits for the physical `iphoneos` SDK, but signing, installation, and device runtime remain unclaimed. Separately, the complete 29,065-function base retail graph now compiles and links for arm64 Simulator with Aurora GX/Metal and SDL UIKit/CoreAudio. Embedding that graph in the real shell, real KartPad data/settings services, Metal gameplay, audio, save, complete races, controller handoff, gyro, and touch-feel acceptance remain open. Evidence: `docs/artifacts/2026-08-30/g14-simulator-shell/` and `docs/artifacts/2026-08-30/g14-full-runtime-link.md`.
