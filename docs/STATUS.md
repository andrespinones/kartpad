...
The local `ref/sunpad` checkout is the direct implementation reference for the mobile touch interface and persistent three-dot menu. A byte-identical, pinned nine-file snapshot now compiles directly in a native arm64 iOS Simulator shell; its exact-snapshot verifier, Mario Kart Classic-input adapter contract, original icon catalog, privacy manifest, and package audit pass. The shell also links the shared checked-memory, scheduler, host, and translated-fixture libraries and carries a bounded startup self-check. The same complete unsigned shell builds and audits for the physical `iphoneos` SDK; signing, installation, and device runtime remain unclaimed. The full 29,637-function retail graph, KartPad-specific menu services, Simulator visual comparison, complete races, and touch-feel acceptance remain open. Mobile completion cannot be claimed before those tests and the PRD's macOS correctness prerequisites pass.
