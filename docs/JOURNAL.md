...
- Built `kartpad_host`, `kartpad_memory`, `kartpad_scheduler`, and `kartpad_g7_translated` as arm64 iOS Simulator static libraries, then compiled the same warning-as-error libraries against the physical `iphoneos` 16.0 SDK. Promoted the UIKit app into the Simulator root graph and linked all four libraries.
