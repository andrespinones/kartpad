...
It then starts from a fresh real menu for each of 13 action checks and requires
...
`Android menu parity passed: lane=<lane> top=8 controls=5 display=2 data=6 actions=13`

This is 21 reachable rows across the consolidated hierarchy plus 13 routed
destinations. The fixture is gated out of translated game-runtime builds and
uses no private data.

## Classification

**Pass for rendered menu hierarchy reachability and 13 representative action
